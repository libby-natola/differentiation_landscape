# Introduction ----
# genomics_R_functions_V2.R
# Written by Darren Irwin in 2015-2016, initially for the purpose of analyzing GBS data
# from Greenish Warblers (Irwin et al. 2016, Molecular Ecology).
# 
# The functions below have been modified from those used in the paper above.
# These are used in the analysis presented in the following paper:
#
# Irwin DE, MilÃ¡ B, Toews DPL, Brelsford A, Kenyon HL, Porter AN, Grossen C, Delmore KE, Alcaide M, Irwin JH. Genomic differentiation and speciation: a comparison of three young avian species pairs. Molecular Ecology, in review. 
#
# The functions are called by a script file called: warbler_GBS_analysis_script_for_Dryad.R
#
...
getWindowedPi <- function(site_pi, pos, window_size, step_size){
  JLD.rolling.mean.pos.pi <- rollapply(pos$position, width=window_size, FUN=mean, by=step_size, align="left")
  JLD.rolling.mean.pi <- matrix(nrow = length(groups), ncol = length(JLD.rolling.mean.pos.pi), dimnames = list(rownames(site_pi), round(JLD.rolling.mean.pos.pi)))
  for (i in 1:length(site_pi[,1])) {
    JLD.rolling.mean.pi[i,] <- rollapply(site_pi[i,], width=window_size, FUN=mean, na.rm = TRUE, by=step_size, align="left")
  } 
  return(list(JLD.rolling.mean.pos.pi=JLD.rolling.mean.pos.pi, JLD.rolling.mean.pi=JLD.rolling.mean.pi))
}
...
getWindowedDxy <- function(Dxy, pos, window_size, step_size){
  JLD.rolling.mean.pos.Dxy <- rollapply(pos$position, width=window_size, FUN=mean, by=step_size, align="left")
  JLD.rolling.mean.Dxy <- matrix(nrow = length(Dxy[,1]), ncol = length(JLD.rolling.mean.pos.Dxy), dimnames = list(rownames(Dxy), round(JLD.rolling.mean.pos.Dxy)))
  for (i in 1:length(Dxy[,1])) {
    JLD.rolling.mean.Dxy[i,] <- rollapply(Dxy[i,], width=window_size, FUN=mean, na.rm = TRUE, by=step_size, align="left")
  }
  return(list(JLD.rolling.mean.pos.Dxy=JLD.rolling.mean.pos.Dxy, JLD.rolling.mean.Dxy=JLD.rolling.mean.Dxy))
}
...
getWindowedWC84_Fst <- function(WC84_Fst_numerator, WC84_Fst_denominator, pos, window_size, step_size){
  JLD.rolling.mean.pos.WC84_Fst <- rollapply(pos$position, width=window_size, FUN=mean, by=step_size, align="left")   # if want partial window on right, add: partial=window_size/2
  JLD.rolling.mean.WC84_Fst <- matrix(nrow = length(WC84_Fst_numerator[,1]), ncol = length(JLD.rolling.mean.pos.WC84_Fst), dimnames = list(rownames(WC84_Fst_numerator), round(JLD.rolling.mean.pos.WC84_Fst)))
  for (i in 1:length(WC84_Fst_numerator[,1])) {
    JLD.rolling.mean.WC84_Fst_numerator <- rollapply(WC84_Fst_numerator[i,], width=window_size, FUN=mean, na.rm = TRUE, by=step_size, align="left")
    JLD.rolling.mean.WC84_Fst_denominator <- rollapply(WC84_Fst_denominator[i,], width=window_size, FUN=mean, na.rm = TRUE, by=step_size, align="left")
    JLD.rolling.mean.WC84_Fst[i,] <- JLD.rolling.mean.WC84_Fst_numerator / JLD.rolling.mean.WC84_Fst_denominator
  }
  return(list(JLD.rolling.mean.pos.WC84_Fst=JLD.rolling.mean.pos.WC84_Fst, JLD.rolling.mean.WC84_Fst=JLD.rolling.mean.WC84_Fst))
}


# make plots for quick inspection of rolling mean results
makeRollingMeanPlots <- function(JLD.rolling.mean.pos.WC84_Fst, JLD.rolling.mean.WC84_Fst, 
                                 JLD.rolling.mean.pos.Dxy, JLD.rolling.mean.Dxy, 
                                 JLD.rolling.mean.pos.pi, JLD.rolling.mean.pi,
  ...
  mean.WC84_Fst.data <- data.frame(mean_position=JLD.rolling.mean.pos.WC84_Fst, mean_WC84_Fst=JLD.rolling.mean.WC84_Fst[1,], populations=rep(rownames(JLD.rolling.mean.WC84_Fst)[1], length(JLD.rolling.mean.pos.WC84_Fst)))
  for (i in 2:length(JLD.rolling.mean.WC84_Fst[,1])) {
    mean.WC84_Fst.data <- rbind(mean.WC84_Fst.data, data.frame(mean_position=JLD.rolling.mean.pos.WC84_Fst, mean_WC84_Fst=JLD.rolling.mean.WC84_Fst[i,], populations=rep(rownames(JLD.rolling.mean.WC84_Fst)[i], length(JLD.rolling.mean.pos.WC84_Fst))))
  }
  mean.Dxy.data <- data.frame(mean_position=JLD.rolling.mean.pos.Dxy, mean_Dxy=JLD.rolling.mean.Dxy[1,], populations=rep(rownames(JLD.rolling.mean.Dxy)[1], length(JLD.rolling.mean.pos.Dxy)))
  for (i in 2:length(JLD.rolling.mean.Dxy[,1])) {
    mean.Dxy.data <- rbind(mean.Dxy.data, data.frame(mean_position=JLD.rolling.mean.pos.Dxy, mean_Dxy=JLD.rolling.mean.Dxy[i,], populations=rep(rownames(JLD.rolling.mean.Dxy)[i], length(JLD.rolling.mean.pos.Dxy))))
  }
  mean.pi.data <- data.frame(mean_position=JLD.rolling.mean.pos.pi, mean_pi=JLD.rolling.mean.pi[1,], taxon=rep(rownames(JLD.rolling.mean.pi)[1], length(JLD.rolling.mean.pos.pi)))
  for (i in 2:length(JLD.rolling.mean.pi[,1])) {
    mean.pi.data <- rbind(mean.pi.data, data.frame(mean_position=JLD.rolling.mean.pos.pi, mean_pi=JLD.rolling.mean.pi[i,], taxon=rep(rownames(JLD.rolling.mean.pi)[i], length(JLD.rolling.mean.pos.pi))))
  }
  # define ggplots:
 ...
 png(file=(paste(base.name,"pi_Fst_Dxy.png")))
...
makePiPlot <- function(pos, site_pi, region.text,
  ...
  graph.data <- rbind(graph.data, data.frame(position=JLD.rolling.mean.pos.pi, pi=amp*JLD.rolling.mean.pi[rownames(JLD.rolling.mean.pi)==groups.to.plot.pi[1],], point_type=rep("windows", length(JLD.rolling.mean.pos.pi)))) 
  ...
  graph.data <- rbind(graph.data, data.frame(position=JLD.rolling.mean.pos.pi, pi=amp*JLD.rolling.mean.pi[rownames(JLD.rolling.mean.pi)==groups.to.plot.pi[2],], point_type=rep("windows", length(JLD.rolling.mean.pos.pi)))) 
  ...
  graph.data <- rbind(graph.data, data.frame(position=JLD.rolling.mean.pos.pi, pi=amp*JLD.rolling.mean.pi[rownames(JLD.rolling.mean.pi)==groups.to.plot.pi[3],], point_type=rep("windows", length(JLD.rolling.mean.pos.pi)))) 
...
makeDxyPlot <- function(pos, Dxy, region.text, 
  ...
  graph.data <- rbind(graph.data, data.frame(position=JLD.rolling.mean.pos.Dxy, Dxy= amp*JLD.rolling.mean.Dxy[rownames(JLD.rolling.mean.Dxy)==groups.to.plot.Dxy[1],], point_type=rep("windows", length(JLD.rolling.mean.pos.Dxy))))
  ...
  graph.data <- rbind(graph.data, data.frame(position=JLD.rolling.mean.pos.Dxy, Dxy= amp*JLD.rolling.mean.Dxy[rownames(JLD.rolling.mean.Dxy)==groups.to.plot.Dxy[2],], point_type=rep("windows", length(JLD.rolling.mean.pos.Dxy))))
  ...
  graph.data <- rbind(graph.data, data.frame(position=JLD.rolling.mean.pos.Dxy, Dxy= amp*JLD.rolling.mean.Dxy[rownames(JLD.rolling.mean.Dxy)==groups.to.plot.Dxy[3],], point_type=rep("windows", length(JLD.rolling.mean.pos.Dxy))))
...
makeFstPlot <- function(pos, WC84_Fst, region.text, 
                        JLD.rolling.mean.pos.WC84_Fst, JLD.rolling.mean.WC84_Fst,
  ...
  graph.data <- rbind(graph.data, data.frame(position=JLD.rolling.mean.pos.WC84_Fst, WC84_Fst= amp*JLD.rolling.mean.WC84_Fst[rownames(JLD.rolling.mean.WC84_Fst)==groups.to.plot.WC84_Fst[1],], point_type=rep("windows", length(JLD.rolling.mean.pos.WC84_Fst))))
  ...
  graph.data <- rbind(graph.data, data.frame(position=JLD.rolling.mean.pos.WC84_Fst, WC84_Fst= amp*JLD.rolling.mean.WC84_Fst[rownames(JLD.rolling.mean.WC84_Fst)==groups.to.plot.WC84_Fst[2],], point_type=rep("windows", length(JLD.rolling.mean.pos.WC84_Fst))))
  ...
  graph.data <- rbind(graph.data, data.frame(position=JLD.rolling.mean.pos.WC84_Fst, WC84_Fst= amp*JLD.rolling.mean.WC84_Fst[rownames(JLD.rolling.mean.WC84_Fst)==groups.to.plot.WC84_Fst[3],], point_type=rep("windows", length(JLD.rolling.mean.pos.WC84_Fst))))
...
makeDetailedChrPlots <- function(pos, 
                                 WC84_Fst, JLD.rolling.mean.pos.WC84_Fst, JLD.rolling.mean.WC84_Fst,
                                 Dxy, JLD.rolling.mean.pos.Dxy, JLD.rolling.mean.Dxy,
                                 site_pi, JLD.rolling.mean.pos.pi, JLD.rolling.mean.pi, 
                                 group.colors.pi, groups.to.plot.pi,
                                 group.colors.Dxy, groups.to.plot.Dxy,
                                 group.colors.WC84_Fst, groups.to.plot.WC84_Fst,
                                 region.text) {
  ...
  png(file=paste0("FstDxypi", chr, ".png"))  # width and height in inches
  label.size <- 1
  ...
  for (j in 1:length(groups.to.plot.WC84_Fst)) {
    ...
    lines(JLD.rolling.mean.pos.WC84_Fst, amp.Fst*JLD.rolling.mean.WC84_Fst[rownames(JLD.rolling.mean.WC84_Fst)==groups.to.plot.WC84_Fst[j],], col=rgb(color.rgb[1], color.rgb[2], color.rgb[3], alpha=line.transparency), lwd=linewidth)
  }
  # plot Dxy:
  row.num <- 2
  for (j in 1:length(groups.to.plot.Dxy)) {
    ...
    new.fig.parameter <- TRUE
    plot(x=NULL, y=NULL, xlim=c(start.bp, end.bp), ylim=c(0, 1.2), yaxp=c(0, 1, n=1), mgp=c(2,0.5,0), ylab=NA, xlab=NA, xaxt='n', yaxt='n', bty='n', las=1, xaxs="i", tcl=-0.25, xpd=NA, cex=1, cex.lab=1.25, cex.axis=0.75)
    mtext(side=4, text=expression("100 * mean " * italic(pi)[B]), col=group.colors.Dxy[j], cex=1, las=1, line=1.5)
    mtext(side=2, text=expression(italic(pi)[B]), col="black", cex=1.2, las=1, line=2)
    axis(side=2, at=c(0,1), tcl=-0.25, las=1, col="black", col.ticks="black", cex=0.75, mgp=c(2,0.5,0))
    ...
    lines(JLD.rolling.mean.pos.Dxy, amp.Dxy*JLD.rolling.mean.Dxy[rownames(JLD.rolling.mean.Dxy)==groups.to.plot.Dxy[j],], col=rgb(color.rgb[1], color.rgb[2], color.rgb[3], alpha=line.transparency), lwd=linewidth)
  }
  # plot pi: 
  row.num <- 3
  for (j in 1:length(groups.to.plot.pi)) {
    ...
    mtext(side=4, text=expression("100 * mean " * italic(pi)[W]), col=group.colors.pi[j], cex=1, las=1, line=1.5)
    mtext(side=2, text=expression(italic(pi)[W] * " "), col="black", cex=1.2, las=1, line=2)
    ...
    lines(JLD.rolling.mean.pos.pi, amp.pi*JLD.rolling.mean.pi[rownames(JLD.rolling.mean.pi)==groups.to.plot.pi[j],], col=rgb(color.rgb[1], color.rgb[2], color.rgb[3], alpha=line.transparency), lwd=linewidth)
  }
...
plotPCA <- function(Fst.filter, Fst.cutoff, groups.to.compare, WC84_Fst, combo.NApass, num_loc_cols, region.text,
  ...
  if (Fst.filter == TRUE) {
    ...
    title.text <- paste0("PCA of ",region.text,": all SNPs", sep="")
  }
  ...
  plot(scores$PC1,scores$PC2, asp=1, pch=23, xlab="PC1", ylab="PC2", main=title.text)
  for (i in 1:length(groups.to.plot.PCA)) {
    selection <- SNP.included$Fst_group == groups.to.plot.PCA[i]
    points(scores$PC1[selection], scores$PC2[selection], pch=23, bg = group.colors.PCA[i])
  }
...
plotGenotypeByIndividual <- function(groups.to.compare, WC84_Fst.cutoff, missing.fraction.max,
  ...
  quartz(title=paste0(region.text,": genotypes Fst>", WC84_Fst.cutoff," loci between ", groups.to.compare, sep=""), width=10, height=6)
...
plotGenomeFstDxyPi <- function(base.name, tag.name, window_size, chromosomes.to.plot, 
  ...
  for (i in 1:length(chromosomes.to.plot)) {
    ...
    chr_fun <- chromosomes.to.analyze[i] # choose chromosome for the loop ### LN CHANGE ###
    region.text_fun <- paste0("Chr",chr_fun,"_whole") ### LN CHANGE ###
    to_load_fun <- paste0(base.name[i],tag.name,region.text_fun,"_window",window_size,"_WindowStats_from_R.R") ### LN CHANGE ###
    load(to_load_fun) # load the WindowStats rolling mean data ### LN CHANGE ###
    #chr.length[i] <- JLD.rolling.mean.pos.Fst[length(JLD.rolling.mean.pos.Fst)]
    chr.length[i] <- JLD.rolling.mean.pos.WC84_Fst[length(JLD.rolling.mean.pos.WC84_Fst)] ### LN CHANGED ###
  }
...
pdf(file=("pi_Fst_Dxy_means_test.pdf"), width=100, height = 100)
 #quartz(title="WC84_Fst, Dxy, and pi for all chromosomes", width=window.width, height=window.height)  # width and height in inches
  ...
  for (row.num in 1:num.rows) {
    ...
    for (order.num in 1:num.chr.in.row) {
      ...
      lapply(to_load,load,.GlobalEnv) ### LN CHANGED ###
      ...
      for (j in 1:length(groups.to.plot.WC84_Fst)) {
        ...
        lines(JLD.rolling.mean.pos.WC84_Fst, JLD.rolling.mean.WC84_Fst[rownames(JLD.rolling.mean.WC84_Fst)==groups.to.plot.WC84_Fst[j],],
              col=rgb(color.rgb[1], color.rgb[2], color.rgb[3], alpha=line_transparency)) ### LN CHANGE ###
        ...
        xx <- c(JLD.rolling.mean.pos.WC84_Fst[1], JLD.rolling.mean.pos.WC84_Fst, JLD.rolling.mean.pos.WC84_Fst[length(JLD.rolling.mean.pos.WC84_Fst)])
        yy <- c(0, JLD.rolling.mean.WC84_Fst[rownames(JLD.rolling.mean.WC84_Fst)==groups.to.plot.WC84_Fst[j],], 0)
        polygon(xx, yy, col=rgb(color.rgb[1], color.rgb[2], color.rgb[3], alpha=transparency_Fst_Dxy), border=NA) ### LN CHANGE ###
      ...
      for (j in 1:length(groups.to.plot.Dxy)) {
        ...
        lines(JLD.rolling.mean.pos.Dxy, JLD.rolling.mean.Dxy[rownames(JLD.rolling.mean.Dxy)==groups.to.plot.Dxy[j],], 
              col=rgb(color.rgb[1], color.rgb[2], color.rgb[3], alpha=line_transparency))
        xx <- c(JLD.rolling.mean.pos.Dxy[1], JLD.rolling.mean.pos.Dxy, JLD.rolling.mean.pos.Dxy[length(JLD.rolling.mean.pos.Dxy)])
        yy <- c(0, JLD.rolling.mean.Dxy[rownames(JLD.rolling.mean.Dxy)==groups.to.plot.Dxy[j],], 0) ### LN CHANGE ###
        polygon(xx, yy, col=rgb(color.rgb[1], color.rgb[2], color.rgb[3], alpha=transparency_Fst_Dxy), border=NA)
      }
      # plot pi: 
...
# plot Genome-wide plot of windowed Fst, Dxy, pi
# read files for each chromosome, and plot Fst, Dxy, and pi for each chromosome in a single window
plotGenomeFst_3plots <- function(base.name, tag.name, window_size, chromosomes.to.plot, 
  ...
  quartz(title="WC84_Fst (3 comparisons) for all chromosomes", width=window.width, height=window.height)  # width and height in inches
  # plot(x=NULL, y=NULL, xlim=c(0,1), ylim=c(0,1), xaxt='n', yaxt='n', xlab=NULL, ylab= NULL, bty='n', las=1)
  Sys.sleep(0.1)  # this is to prevent a bug that misplaces the y axis labels in the first plot
  num.rows <- sum(!is.na(chr.in.row[,1]))
  new.fig.parameter <- FALSE
  for (row.num in 1:num.rows) {
    num.chr.in.row <- sum(!is.na(chr.in.row[row.num,]))
    for (order.num in 1:num.chr.in.row) {
      ...
      # top plot:
      plot(x=NULL, y=NULL, xlim=c(0, plot.bp.length), ylim=c(0, 1.2*max_Fst_axis), yaxp=c(0, max_Fst_axis, n=1), ylab=NA, xlab=NA, xaxt='n', bty='n', las=1, xaxs="i", tcl=-0.25, xpd=NA, cex=1, cex.lab=1.55, cex.axis=0.75)
      text(-2000000, 1.4*max_Fst_axis, chr.in.row[row.num,order.num], pos=4, cex=2, xpd=NA)
      j <- 1  # plot the first comparison
      color.rgb <- col2rgb(group.colors.WC84_Fst[j]) /255   # divide to convert color scales to 0-1
      lines(JLD.rolling.mean.pos.WC84_Fst, JLD.rolling.mean.WC84_Fst[rownames(JLD.rolling.mean.WC84_Fst)==groups.to.plot.WC84_Fst[j],],
            col=rgb(color.rgb[1], color.rgb[2], color.rgb[3], alpha=line_transparency), lwd=0.5)
      xx <- c(JLD.rolling.mean.pos.WC84_Fst[1], JLD.rolling.mean.pos.WC84_Fst, JLD.rolling.mean.pos.WC84_Fst[length(JLD.rolling.mean.pos.WC84_Fst)])
      yy <- c(0, JLD.rolling.mean.WC84_Fst[rownames(JLD.rolling.mean.WC84_Fst)==groups.to.plot.WC84_Fst[j],], 0)
      polygon_color <- rgb(color.rgb[1], color.rgb[2], color.rgb[3], alpha=transparency)
      polygon(xx, yy, col=polygon_color, border=polygon_color)
      new.fig.parameter <- TRUE
      
      # middle plot:
      if (order.num==1) {
        y_label <- expression(italic('F')[ST]*"  ")
      } else {
        y_label <- NA
      }
      par(fig=c(fig.left.loc, fig.right.loc, 1-(top.margin+2*plot.height+(row.num-1)*(row.height+row.space)), 1-(top.margin+plot.height+(row.num-1)*(row.height+row.space))), new = TRUE, mai=c(0,0,0,0))  # mai sets the margins of the plot to zero, matching
      plot(x=NULL, y=NULL, xlim=c(0, plot.bp.length), ylim=c(0, 1.2*max_Fst_axis), yaxp=c(0, max_Fst_axis, n=1), ylab=y_label, xlab=NA, xaxt='n', bty='n', las=1, xaxs="i", tcl=-0.25, xpd=NA, cex=1, cex.lab=2.5, cex.axis=0.75)
      j <- 2 # plot the second comparison
      color.rgb <- col2rgb(group.colors.WC84_Fst[j]) /255   # divide to convert color scales to 0-1
      lines(JLD.rolling.mean.pos.WC84_Fst, JLD.rolling.mean.WC84_Fst[rownames(JLD.rolling.mean.WC84_Fst)==groups.to.plot.WC84_Fst[j],],
            col=rgb(color.rgb[1], color.rgb[2], color.rgb[3], alpha=line_transparency), lwd=0.5)
      xx <- c(JLD.rolling.mean.pos.WC84_Fst[1], JLD.rolling.mean.pos.WC84_Fst, JLD.rolling.mean.pos.WC84_Fst[length(JLD.rolling.mean.pos.WC84_Fst)])
      ...
      par(fig=c(fig.left.loc, fig.right.loc, 1-(top.margin+3*plot.height+(row.num-1)*(row.height+row.space)), 1-(top.margin+2*plot.height+(row.num-1)*(row.height+row.space))), new = TRUE, mai=c(0,0,0,0))  # mai sets the margins of the plot to zero, matching
      plot(x=NULL, y=NULL, xlim=c(0, plot.bp.length), ylim=c(0, 1.2*max_Fst_axis), yaxp=c(0, max_Fst_axis, n=1), ylab=NA, xlab=NA, xaxt='n', bty='n', las=1, xaxs="i", tcl=-0.25, xpd=NA, cex=1, cex.lab=1.55, cex.axis=0.75)
      j <- 3 # plot the second comparison
      color.rgb <- col2rgb(group.colors.WC84_Fst[j]) /255   # divide to convert color scales to 0-1
      lines(JLD.rolling.mean.pos.WC84_Fst, JLD.rolling.mean.WC84_Fst[rownames(JLD.rolling.mean.WC84_Fst)==groups.to.plot.WC84_Fst[j],],
            col=rgb(color.rgb[1], color.rgb[2], color.rgb[3], alpha=line_transparency), lwd=0.5)
      xx <- c(JLD.rolling.mean.pos.WC84_Fst[1], JLD.rolling.mean.pos.WC84_Fst, JLD.rolling.mean.pos.WC84_Fst[length(JLD.rolling.mean.pos.WC84_Fst)])
...
compileWindowedStats <- function(base.name, tag.name, chromosomes.to.combine, window_size) {
  genome.JLD.rolling.mean.positions <- NULL
  genome.JLD.rolling.mean.WC84_Fst <- NULL
  genome.JLD.rolling.mean.Dxy <- NULL
  genome.JLD.rolling.mean.pi <- NULL
  for (i in 1:length(chromosomes.to.combine)) {
    ...
    chromosome.ID.vector <- rep(chromosomes.to.combine[i], times=length(JLD.rolling.mean.pos.WC84_Fst))
    genome.JLD.rolling.mean.positions <- cbind(genome.JLD.rolling.mean.positions, rbind(chromosome.ID.vector, JLD.rolling.mean.pos.WC84_Fst))
    genome.JLD.rolling.mean.WC84_Fst <- cbind(genome.JLD.rolling.mean.WC84_Fst, JLD.rolling.mean.WC84_Fst)
    genome.JLD.rolling.mean.Dxy <- cbind(genome.JLD.rolling.mean.Dxy, JLD.rolling.mean.Dxy)
    genome.JLD.rolling.mean.pi <- cbind(genome.JLD.rolling.mean.pi, JLD.rolling.mean.pi)
  }
  return(list(positions=genome.JLD.rolling.mean.positions, 
              WC84_Fst=genome.JLD.rolling.mean.WC84_Fst, 
              Dxy=genome.JLD.rolling.mean.Dxy, 
              pi=genome.JLD.rolling.mean.pi))
}


# For compiled windowed data ----

# Make a plot of windowed Fst vs. 1 - (pi/Dxy) 
# using compiled genome-wide info, and conduct pearson's correlation test:
plotFst_1minusPioverDxy <- function(group1, group2, autosome.genome.rolling.stats, method) {
  comp <- paste0(group1, "_", group2)
  quartz(title=paste0("WC84_Fst vs. 1-(pi/Dxy), based on all autosomes, ", comp, sep=""), width=4, height=4)
  WC84_Fst.row.choice <- which(rownames(autosome.genome.rolling.stats$WC84_Fst) == comp)
  WC84_Fst <- as.vector(autosome.genome.rolling.stats$WC84_Fst[WC84_Fst.row.choice,])
  Dxy.row.choice <- which(rownames(autosome.genome.rolling.stats$Dxy) == comp)
  ...
  plot(WC84_Fst, y, 
       ...
       ylab=paste0("windowed 1-(mean_pi/Dxy) for ", comp, sep=""))
  lines(c(0,1), c(0,1)) # plots 1:1 line
  result <- cor.test(WC84_Fst, y,
                     method=method)
  return(result)
}
...
plotFst_density <- function(comp1, comp2, comp3, 
                            autosome.genome.rolling.stats) {
  ...
  vector2 <- as.vector(autosome.genome.rolling.stats$WC84_Fst[row.choice.2,])
  vector3 <- as.vector(autosome.genome.rolling.stats$WC84_Fst[row.choice.3,])
  quartz(title=paste0("Histograms of Fst in three comparisons"), width=4, height=3)
  label.size <- 1.5
  Fst_axis_range <- c(-0.05, 1)
  Density_axis_range <- c(-2, 17)
  median_line_height <- 0.9*Density_axis_range[2]
  
  par(fig=c(0.1,0.9,0.7,0.9), mai=c(0,0,0,0))
  d1 <- density(vector1, from=min(vector1), to=max(vector1))
  plot(x=NULL, y=NULL, xlim=Fst_axis_range, ylim=Density_axis_range, xlab=NA, ylab=NA, bty='n', xaxt='n', yaxt='n')
  polygon(d1, col=comp1[3], border=comp1[3], xlab=NA)
  # draw line for median
  median_Fst <- median(vector1)
  lines(x=c(median_Fst,median_Fst), y=c(0, median_line_height), type="l", lty="solid")
  # draw line for 99th percentile
  x_loc <- quantile(vector1, 0.99)
  lines(x=c(x_loc,x_loc), y=c(0, median_line_height), type="l", lty="dotted")
  
  par(fig=c(0.1,0.9,0.5,0.7), mai=c(0,0,0,0), new=TRUE)
  d2 <- density(vector2, from=min(vector2), to=max(vector2))
  plot(x=NULL, y=NULL, xlim=Fst_axis_range, ylim=Density_axis_range, xlab=NA, ylab=NA, bty='n', xaxt='n', yaxt='n')
  polygon(d2, col=comp2[3], border=comp2[3], xlab=NA)
  # draw line for median
  median_Fst <- median(vector2)
  lines(x=c(median_Fst,median_Fst), y=c(0, median_line_height), type="l")
  mtext(side=2, text=expression('Density'), col="black", cex=1.5, las=0, line=0)
  # draw line for 99th percentile
  x_loc <- quantile(vector2, 0.99)
  lines(x=c(x_loc,x_loc), y=c(0, median_line_height), type="l", lty="dotted")
  
  par(fig=c(0.1,0.9,0.3,0.5), mai=c(0,0,0,0), new=TRUE)
  d3 <- density(vector3, from=min(vector3), to=max(vector3))
  plot(x=NULL, y=NULL, xlim=Fst_axis_range, ylim=Density_axis_range, xlab=NA, ylab=NA, bty='n', yaxt='n', mgp=c(2,0.5,0))
  polygon(d3, col=comp3[3], border=comp3[3])
  # draw line for median
  median_Fst <- median(vector3)
  lines(x=c(median_Fst,median_Fst), y=c(0, median_line_height), type="l")
  mtext(side=1, text=expression(italic('F')[ST]), col="black", cex=1.5, las=1, line=2.5)
  # draw line for 99th percentile
  x_loc <- quantile(vector3, 0.99)
  lines(x=c(x_loc,x_loc), y=c(0, median_line_height), type="l", lty="dotted")
}
...
plots3Dxy_Dxy <- function(comp1, comp2, comp3, 
  ...
  plot(x=vector1, y=vector3, xlim=c(0,high.Dxy), xaxp=c(0,high.Dxy,2), ylim=c(0,high.Dxy), yaxp=c(0,high.Dxy,2), 
       asp=1, pch=16, cex=0.25, cex.axis=0.9, tcl=-0.5, xlab=NA, ylab=NA, mgp=c(3,0.5,0))
  title(xlab=bquote(italic(pi)[B] * ", " * phantom(italic(.(comp1[2])) * " to " * italic(.(comp1[1])))), line=2.5, cex.lab=label.size)
  title(xlab=bquote(phantom(italic(pi)[B] * ", ") * italic(.(comp1[2])) * " to " * italic(.(comp1[1]))), line=2.5, cex.lab=label.size, col.lab=comp1[3])
  title(ylab=bquote(italic(pi)[B] * ", " * phantom(italic(.(comp3[1])) * " to " * italic(.(comp3[2])))), line=2, cex.lab=label.size)
  title(ylab=bquote(phantom(italic(pi)[B] * ", ") * italic(.(comp3[1])) * " to " * italic(.(comp3[2]))), line=2, cex.lab=label.size, col.lab=comp3[3])
  # second plot:
  plot(x=vector2, y=vector1, xlim=c(0,high.Dxy), xaxp=c(0,high.Dxy,2), ylim=c(0,high.Dxy), yaxp=c(0,high.Dxy,2), 
       asp=1, pch=16, cex=0.25, cex.axis=0.9, tcl=-0.5, xlab=NA, ylab=NA, mgp=c(3,0.5,0))
  title(xlab=bquote(italic(pi)[B] * ", " * phantom(italic(.(comp2[1])) * " to " * italic(.(comp2[2])))), line=2.5, cex.lab=label.size)
  title(xlab=bquote(phantom(italic(pi)[B] * ", ") * italic(.(comp2[1])) * " to " * italic(.(comp2[2]))), line=2.5, cex.lab=label.size, col.lab=comp2[3])
  title(ylab=bquote(italic(pi)[B] * ", " * phantom(italic(.(comp1[2])) * " to " * italic(.(comp1[1])))), line=2, cex.lab=label.size)
  title(ylab=bquote(phantom(italic(pi)[B] * ", ") * italic(.(comp1[2])) * " to " * italic(.(comp1[1]))), line=2, cex.lab=label.size, col.lab=comp1[3])
  # third plot:
  plot(x=vector3, y=vector2, xlim=c(0,high.Dxy), xaxp=c(0,high.Dxy,2), ylim=c(0,high.Dxy), yaxp=c(0,high.Dxy,2), 
       asp=1, pch=16, cex=0.25, cex.axis=0.9, tcl=-0.5, xlab=NA, ylab=NA, mgp=c(3,0.5,0))
  title(xlab=bquote(italic(pi)[B] * ", " * phantom(italic(.(comp3[1])) * " to " * italic(.(comp3[2])))), line=2.5, cex.lab=label.size)
  title(xlab=bquote(phantom(italic(pi)[B] * ", ") * italic(.(comp3[1])) * " to " * italic(.(comp3[2]))), line=2.5, cex.lab=label.size, col.lab=comp3[3])
  title(ylab=bquote(italic(pi)[B] * ", " * phantom(italic(.(comp2[1])) * " to " * italic(.(comp2[2])))), line=2, cex.lab=label.size)
  title(ylab=bquote(phantom(italic(pi)[B] * ", ") * italic(.(comp2[1])) * " to " * italic(.(comp2[2]))), line=2, cex.lab=label.size, col.lab=comp2[3])
  # statistical tests
  test1 <- cor.test(vector1, vector3, method=cor.method)
  test2 <- cor.test(vector2, vector1, method=cor.method)
  test3 <- cor.test(vector3, vector2, method=cor.method)
...
plots3meanPi_meanPi <- function(comp1, comp2, comp3, cor.method, 
  ...
  plot(x=vector1, y=vector3, xlim=c(0,high.pi), xaxp=c(0,high.pi,2), ylim=c(0,high.pi), yaxp=c(0,high.pi,2), 
       asp=1, pch=16, cex=0.25, cex.axis=0.9, tcl=-0.5, xlab=NA, ylab=NA, mgp=c(3,0.5,0))
  title(xlab=bquote("Mean " * italic(pi)[W] * ", " * phantom(italic(.(comp1[1])) * " & " * italic(.(comp1[2])))), line=2.5, cex.lab=label.size)
  title(xlab=bquote(phantom("Mean " * italic(pi)[W] * ", ") * italic(.(comp1[1]) * " & " * italic(.(comp1[2])))), line=2.5, cex.lab=label.size, col.lab=comp1[3])
  title(ylab=bquote("Mean " * italic(pi)[W] * ", " * phantom(italic(.(comp3[1])) * " & " * italic(.(comp3[2])))), line=2.5, cex.lab=label.size)
  title(ylab=bquote(phantom("Mean " * italic(pi)[W] * ", ") * italic(.(comp3[1]) * " & " * italic(.(comp3[2])))), line=2.5, cex.lab=label.size, col.lab=comp3[3])
  # second plot:
  plot(x=vector2, y=vector1, xlim=c(0,high.pi), xaxp=c(0,high.pi,2), ylim=c(0,high.pi), yaxp=c(0,high.pi,2), 
       asp=1, pch=16, cex=0.25, cex.axis=0.9, tcl=-0.5, xlab=NA, ylab=NA, mgp=c(3,0.5,0))
  title(xlab=bquote("Mean " * italic(pi)[W] * ", " * phantom(italic(.(comp2[1])) * " & " * italic(.(comp2[2])))), line=2.5, cex.lab=label.size)
  title(xlab=bquote(phantom("Mean " * italic(pi)[W] * ", ") * italic(.(comp2[1]) * " & " * italic(.(comp2[2])))), line=2.5, cex.lab=label.size, col.lab=comp2[3])
  title(ylab=bquote("Mean " * italic(pi)[W] * ", " * phantom(italic(.(comp1[1])) * " & " * italic(.(comp1[2])))), line=2.5, cex.lab=label.size)
  title(ylab=bquote(phantom("Mean " * italic(pi)[W] * ", ") * italic(.(comp1[1]) * " & " * italic(.(comp1[2])))), line=2.5, cex.lab=label.size, col.lab=comp1[3])
  # third plot:
  plot(x=vector3, y=vector2, xlim=c(0,high.pi), xaxp=c(0,high.pi,2), ylim=c(0,high.pi), yaxp=c(0,high.pi,2), 
       asp=1, pch=16, cex=0.25, cex.axis=0.9, tcl=-0.5, xlab=NA, ylab=NA, mgp=c(3,0.5,0))
  title(xlab=bquote("Mean " * italic(pi)[W] * ", " * phantom(italic(.(comp3[1])) * " & " * italic(.(comp3[2])))), line=2.5, cex.lab=label.size)
  title(xlab=bquote(phantom("Mean " * italic(pi)[W] * ", ") * italic(.(comp3[1]) * " & " * italic(.(comp3[2])))), line=2.5, cex.lab=label.size, col.lab=comp3[3])
  title(ylab=bquote("Mean " * italic(pi)[W] * ", " * phantom(italic(.(comp2[1])) * " & " * italic(.(comp2[2])))), line=2.5, cex.lab=label.size)
  title(ylab=bquote(phantom("Mean " * italic(pi)[W] * ", ") * italic(.(comp2[1]) * " & " * italic(.(comp2[2])))), line=2.5, cex.lab=label.size, col.lab=comp2[3])
  test1 <- cor.test(vector1, vector3, method=cor.method)
  test2 <- cor.test(vector2, vector1, method=cor.method)
  test3 <- cor.test(vector3, vector2, method=cor.method) 
  return(list(test1=test1, test2=test2, test3=test3))
